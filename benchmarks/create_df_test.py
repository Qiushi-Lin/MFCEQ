...
def main():
    ...
    for map_size, form_size, num_agents in configs['test_df_settings']:
        maps, forms = [], []
        ...
        for i in range(num_maps):
            ...
            file_name = f"./test_set/maps/ms{map_size}_fs{form_size}_na{num_agents}_df_{i}.map"
            save_map_file(map_size, map_size, file_name, grid_map)
        for i in range(num_forms):
            starts, goals_a = generate_random_form(num_agents, map_size, map_size, form_size, form_size)     
            _, goals_b = generate_random_form(num_agents, map_size, map_size, form_size, form_size)     
            forms.append((starts, goals_a, goals_b))
            file_name = f"./test_set/agents/ms{map_size}_fs{form_size}_na{num_agents}_df_{i}_a.agent"
            save_form_file(file_name, map_size, map_size, form_size, form_size, num_agents, starts, goals_a)
            file_name = f"./test_set/agents/ms{map_size}_fs{form_size}_na{num_agents}_df_{i}_b.agent"
            save_form_file(file_name, map_size, map_size, form_size, form_size, num_agents, starts, goals_b)
        instances = {'maps': maps, 'forms': forms}
        file_name = f"./test_set/ms{map_size}_fs{form_size}_na{num_agents}_df.pth"
